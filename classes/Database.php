    ...
    class Database {
        ...
        private function __construct () {
	       ...
	       if ($this->db = new SQLite3('db/wh.db')) {
            	if ($newDB) {
                    #holes
            	    $this->db->exec('CREATE TABLE holes (system TEXT, name TEXT, reporter INT, reported INT, sig TEXT, status INT);');
                    ...
                    #loot sheet master
                    $this->db->exec('CREATE TABLE loots (hole TEXT, fc INT, date INT, sites INT, isk INT, status INT, proof TEXT);');
